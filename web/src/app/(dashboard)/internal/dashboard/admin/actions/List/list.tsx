...
async function getData(apiParams: ApiParams): Promise<EmailTemplate[]> {
  try {
    ...
    const resp: { emailTemplates: EmailTemplate[] } = await response.json()
...
export default async function ActionList(props: WithJwtProps) {
  const jwt = props.jwt
